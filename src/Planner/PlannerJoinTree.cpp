    ...
    bool wrap_read_columns_in_subquery)
{
    ...
    const auto & alias_columns_expressions = table_expression_data.getAliasColumnsExpressions();
    if (!alias_columns_expressions.empty())
    {
        ActionsDAGPtr alias_columns_actions_dag = std::make_shared<ActionsDAG>(query_plan.getCurrentDataStream().header.getColumnsWithTypeAndName());
        auto action_dag_outputs = alias_columns_actions_dag->getInputs();
        for (const String & alias_columns_name : table_expression_data.getAliasColumnsNames())
        {
            auto it = alias_columns_expressions.find(alias_columns_name);
            if (it == alias_columns_expressions.end())
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Alias columns expressions for '{}' is not found", alias_columns_name);

            const ActionsDAGPtr & alias_column_actions_dag = it->second;
            ...
            alias_columns_actions_dag->mergeNodes(std::move(*alias_column_actions_dag));
        }

        for (const auto * output_node : action_dag_outputs)
            alias_columns_actions_dag->addOrReplaceInOutputs(*output_node);
        alias_columns_actions_dag->removeUnusedActions(false);

        auto alias_column_step = std::make_unique<ExpressionStep>(query_plan.getCurrentDataStream(), alias_columns_actions_dag);
